...
export function PresetPicker() {
  ...
  return <div className="preset-picker" ref={root} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setOpen(false); }} onKeyDown={(event) => {
    ...
    {open && <div className="preset-popover"><input ref={search} className="text-input" aria-label="Search styles" placeholder="Search styles…" value={query} onChange={(event) => setQuery(event.target.value)} /><div id="preset-options" role="menu" aria-label="Choose a style">
